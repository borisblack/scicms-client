...
SciCMS Client представляет собой React-приложение, написанное на Typescript. Для сборки используется [Vite](https://vitejs.dev).
В качестве UI фреймворка выбран [Ant Design](https://ant.design).

Перед первым запуском необходимо установить зависимости командой `npm install` (в рабочей директории проекта).

Доступны следующие команды:
- `npm run dev` - локальный запуск; приложение будет открыто в браузере по адресу [http://localhost:3000](http://localhost:3000); при внесении изменений в исход код страница автоматически перезагрузится;
- `npm test` - запуск тестов;
- `npm run build` - сборка оптимизированного бандла для промышленной эксплуатации.
...
- `VITE_CORE_URL` - URL сервера SciCMS Core (значение по умолчанию - `http://localhost:8079`, соответствует локально запущенному экземпляру сервера);
- `VITE_I18N_LNG` - язык интерфейса (значение по умолчанию - `en-US`); в настоящий момент доступны значения: `en-US` и `ru-RU`.

Переменные могут быть заданы в `.env` файлах для различных окружений, подробнее см. в [документации Vite](https://vitejs.dev/guide/env-and-mode.html).
