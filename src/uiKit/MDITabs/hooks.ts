...
export function useNewMDIContext<T>(initialItems: MDITabObservable<T>[]): MDIContext<T> {
  ...
  const updateTab = useCallback(
    (key: string, data: T, newKey?: string) => {
      ...
      // Update active key
      if (key === activeKey && newKey != null && newKey !== key) setActiveKey(newKey)
  ...
  const updateActiveTab = useCallback(
    (data: T, newKey?: string) => {
      ...
      // Update active key
      if (newKey != null && newKey !== activeKey) setActiveKey(newKey)
  ...
  const closeTab = useCallback(
    (key: string, remove?: boolean) => {
      ...
      // Set new active key
      if (key === activeKey && newItems.length > 0) setActiveKey(newItems[newItems.length - 1].key)
  ...
  const closeActiveTab = useCallback(
    (remove?: boolean) => {
      ...
      // Set new active key
      if (newItems.length > 0) setActiveKey(newItems[newItems.length - 1].key)
