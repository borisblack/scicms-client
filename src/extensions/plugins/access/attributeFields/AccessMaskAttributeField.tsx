...
export const AccessMaskAttributeField: FC<CustomAttributeFieldContext> = ({
  itemTab: dataWrapper,
  form,
  attrName,
  attribute,
  ...
  const initialValue = useMemo(
    () => value ?? (attribute.defaultValue == null ? 0 : parseInt(attribute.defaultValue)),
  ...
  return (
    <>
      ...
      <Checkbox
        id={`${uniqueKey}#${attrName}.read`}
        defaultChecked={getBit(initialValue, 0)}
        onChange={evt => handleChange(evt.target.checked, 0)}
      >
        <b>R</b>ead
      </Checkbox>
      <Checkbox
        id={`${uniqueKey}#${attrName}.write`}
        defaultChecked={getBit(initialValue, 1)}
        onChange={evt => handleChange(evt.target.checked, 1)}
      >
        <b>W</b>rite
      </Checkbox>
      <Checkbox
        id={`${uniqueKey}#${attrName}.create`}
        defaultChecked={getBit(initialValue, 2)}
        onChange={evt => handleChange(evt.target.checked, 2)}
      >
        <b>C</b>reate
      </Checkbox>
      <Checkbox
        id={`${uniqueKey}#${attrName}.delete`}
        defaultChecked={getBit(initialValue, 3)}
        onChange={evt => handleChange(evt.target.checked, 3)}
      >
        <b>D</b>elete
      </Checkbox>
      <Checkbox
        id={`${uniqueKey}#${attrName}.administration`}
        defaultChecked={getBit(initialValue, 4)}
        onChange={evt => handleChange(evt.target.checked, 4)}
      >
        <b>A</b>dministration
      </Checkbox>
    </>
