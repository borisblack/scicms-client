...
export function Indexes({itemTab: dataWrapper, buffer, onBufferChange}: CustomComponentContext) {
  ...
  const initialNamedIndexes = useMemo((): NamedIndex[] => {
    ...
    let namedIndexes = Object.keys(indexes).map(indexName => ({name: indexName, ...indexes[indexName]}))
    ...
    if (item.name !== ITEM_TEMPLATE_ITEM_NAME && namedIndexes.length > 0 && !isNew) {
      ...
      for (const itemTemplateName of data.includeTemplates) {
        ...
        for (const excludedIndexName in itemTemplate.spec.indexes) excludedIndexNameSet.add(excludedIndexName)
      }
  ...
  const handleNamedIndexesChange = useCallback(
    (newNamedIndexes: NamedIndex[]) => {
  ...
  const handleIndexFormFinish = useCallback(
    (values: NamedIndex) => {
      ...
      else handleNamedIndexesChange([...namedIndexes, {...parsedValues}])
