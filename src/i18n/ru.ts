...
const ru = {
  translation: {
    ...
    'Cannot resolve type. Eihther Source or Formula field must be set.':
      'Невозможно определить тип. Должно быть задано поле Источник или Формула.',
    ...
    'Error fetching security config': 'Ошибка получения конфигурации безопасности',
    ...
    Filters: 'Фильтры',
    'First line is a heading': 'Первая строка - заголовок',
    Format: 'Формат',
    Formula: 'Формула',
    ...
    'Invalid rule: [%s]. Any value cannot be allowed for any identity.':
      'Некорректное правило: [%s]. Любое значение не может быть разрешено для любого субъекта.',
    'Invalid identities definition: [%s]. All identities are allowed, but the user and/or role lists are not empty.':
      'Некорректное определение субъектов. Разрешены все субъекты, но списки пользователей и/или ролей не пусты.',
    'Invalid identities definition: [%s]. Only one star placeholder is allowed.':
      'Некорректное определение субъектов: [%s]. Разрешено использование только одного заполнителя *.',
    ...
    'Locking error': 'Ошибка блокировки',
    ...
    number: 'число',
    OAuth2: 'OAuth2',
    'OAuth2 Provider': 'Провайдер OAuth2',
    ...
    'Please confirm new password': 'Пожалуйста подтвердите новый пароль',
    ...
    'Promotion error': 'Ошибка перемещения',
    ...
    'Requested resource has been deleted and is no longer available':
      'Запрашиваемый ресурс удален и больше не доступен',
    ...
    'RLS help description':
      "Каждое правило начинается с новой строки.\nПустые строки и пробелы вне кавычек игнорируются.\nНеактивные правила начинаются с символа #.\nПравило состоит из значения поля в одиночных кавычках, знака двоеточия и списка пользователей и/или ролей через запятую. Имя роли должно начинаться с префикса '@role:'.\nСимвол звездочки слева от двоеточего означает любое значение поля. Звездочка справа означает любого пользователя или любую роль. Одновременно слева и справа звездочка стоять не может.",
    Role: 'Роль',
    Roles: 'Роли',
    'Row-level security': 'Доступ на уровне строк',
    'Rule parse error': 'Ошибка разбора правила',
    Rules: 'Правила',
    ...
    SPREADSHEET: 'Электронная таблица (файл xls, xlsx)',
    ...
    Select: 'Выбрать',
    'Select attributes from template': 'Выбрать атрибуты из шаблона',
    ...
    'Service is unavailable. Server is not ready to process the request':
    ...
    'String must start with a letter and contain no whitespaces':
      'Строка должна начинаться с буквы и не содержать пробелов',
    'String must start with a lowercase letter and contain no whitespaces':
      'Строка должна начинаться с буквы нижнего регистра и не содержать пробелов',
    Suffix: 'Суффикс',
    ...
    'Template attributes': 'Аттрибуты шаблона',
    Temporal: 'Дата/время',
