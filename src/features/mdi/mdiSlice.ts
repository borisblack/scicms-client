...
const mdiSlice = createSlice({
  ...
  reducers: {
    ...
    update(draft: Draft<MDIState<any>>, action: PayloadAction<MDIAction<any>>) {
      ...
      if (newKey != null) draft.activeKey = newKey
    },
    ...
    close(draft: Draft<MDIState<any>>, action: PayloadAction<MDIAction<any>>) {
      ...
      if (key === activeKey) draft.activeKey = items.length > 0 ? items[items.length - 1].key : undefined
    },
...
export const {initItems, setActiveKey, open, update, updateActive, close, closeActive, reset} = mdiSlice.actions
