...
Строковые поля датасета имеют свойство **Доступ на уровне строк**, которое содержит описание правил RLS.
...
Правило состоит из значения поля в одиночных кавычках, знака двоеточия и списка пользователей и/или ролей через запятую. Имя роли должно начинаться с префикса '@role:'.Символ звездочки слева от двоеточего означает любое значение поля. Звездочка справа означает любого пользователя или любую роль. Одновременно слева и справа звездочка стоять не может.
Примеры правил RLS:
- `'value1': user1, user2, @role:ROLE_MANAGER` - доступ для строк со значением поля 'value1' разрешен пользователям `user1`, `user2`, а также всем пользователям с ролью `ROLE_MANAGER`;
- `*: user3, user4` - пользователям `user3`, `user4` разрешен доступ к строке с любым значением поля;
- `# 'value2': user5` - закомментированное (неактивное) правило;
- `value3': *` - доступ для строк со значением поля 'value3' разрешен пользователям.
